...
export default class StorageService {
    constructor(context) {
        ...
        this.storagePath = path.resolve(process.cwd(), 'data/storage');
        this.publicBaseUrl = this.config.public_url || 'http://localhost:3000/files';
    }

    async init() {
        if (!fs.existsSync(this.storagePath)) {
            this.logger.debug('infra:storage', `Creando directorio: ${this.storagePath}`);
    ...
    async storeFromUrl(url, options = {}) {
        ...
        const traceId = (typeof this.logger.getCorrelationId === 'function')
        ...
        try {
            ...
            if (options.type === 'image' || options.type === 'sticker') {
                ...
                if (transformStream && options.optimizeFor === 'wa') {
                    extension = 'webp';
                }
            }
            ...
            this.logger.error('infra:storage', 'Error descargando/procesando archivo', { url, error: error.message });
            throw error;
        }
    }
    ...
    async delete(fileName) {
        try {
            ...
            this.logger.warn('infra:storage', 'Error eliminando archivo (puede que no exista)', { fileName });
        }
    }
    ...
    _createImagePipeline(options) {
        try {
            ...
            this.logger.error('infra:storage', 'Error inicializando Sharp', { err: err.message });
            return null;
        }
    }
